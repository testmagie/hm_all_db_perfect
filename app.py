        ...
        if request.method == 'POST':
            ...
            try:
                with get_db() as conn:
                    ...
                    cursor.execute("""
                        INSERT INTO lab_admins (name, specialization, username, password)
                        VALUES (?, ?, ?, ?)
                    """, (data['name'], data['specialization'], data['username'], hashed_password))
...
def view_labadmins():
    ...
    cursor.execute("SELECT * FROM lab_admins")
...
@app.route('/labadmin/upload', methods=['POST'])
